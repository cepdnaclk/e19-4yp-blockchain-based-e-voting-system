...
export const getVoterProfile = async (req: Request, res: Response) => {
  try {
    ...
    const voterResult = await dbQuery({
      query: 'SELECT voter_id, name, email, created_at FROM voters WHERE voter_id = $1',
      params: [voter_id],
    });
    ...
    const votingHistoryResult = await dbQuery({
      query: 'SELECT election_name, voted_at, status FROM voting_history WHERE voter_id = $1 ORDER BY voted_at DESC',
