...
chaincodeInvoke() {
    ...
    peer chaincode invoke -o localhost:7050 \
        ...
        --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_ORG2_CA   \
        -c '{"function": "postHash","Args":["Test Type", "Test Hash"]}'
}

chaincodeQuery() {
    setGlobalsForPeer0Org2

    # Query The Dummy Data
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "getHistory","Args":["test2"]}'
}
